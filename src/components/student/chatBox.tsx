          ...
          if (data && data.message) {
            ...
            setMessages((prev) => [
              ...prev,
              {
                id: Date.now() + 2,
                fromMe: false,
                text: 'AI did not respond properly. Please try again.',
                time: formatTime(),
              },
            ]);
          }
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 bg-gray-50 rounded-t-xl">
              <div className="flex items-center gap-2">
                ...
                {currentChatSessionId && (
                  <span className="text-xs text-gray-500">Session: {currentChatSessionId}</span>
                )}
              </div>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex mb-2 ${msg.fromMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`px-3 py-2 rounded-2xl max-w-[70%] text-sm shadow-sm ${msg.fromMe ? 'bg-blue-500 text-white rounded-br-md' : 'bg-gray-100 text-gray-800 rounded-bl-md'}`}>
                    {msg.text}
                    <div className={`text-[10px] mt-1 opacity-60 ${msg.fromMe ? 'text-right' : 'text-left'}`}>
                      {msg.time}
                    </div>
                  </div>
                </div>
            ...
            <div className="p-2 border-t border-gray-200 bg-gray-50 rounded-b-xl flex gap-2">
              ...
              >
                <svg width="20" height="20" fill="none" viewBox="0 0 20 20">
                  <path fill="currentColor" d="M2.93 2.93a2.5 2.5 0 0 1 2.6-.6l11.25 4.5c2.02.81 2.02 3.62 0 4.43l-11.25 4.5a2.5 2.5 0 0 1-3.37-2.6l.5-3.75a.625.625 0 0 1 1.24.17l-.5 3.75a1.25 1.25 0 0 0 1.68 1.3l11.25-4.5a1.25 1.25 0 0 0 0-2.36l-11.25-4.5a1.25 1.25 0 0 0-1.68 1.3l.5 3.75a.625.625 0 0 1-1.24.17l-.5-3.75a2.5 2.5 0 0 1 .77-2.4Z"/>
                </svg>
              </Button>
            </div>
          </motion.div>
